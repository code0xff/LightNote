...
- The top clearance for the fixed nav/aside lives on `.editor-shell` (`pt-44 lg:pt-16`), not on the editor body, because the title block now occupies that space and sharing mode swaps the field for static text. It must match the aside's height below `lg` (`h-28`), where the document list is a fixed-height strip under the nav rather than a sidebar.

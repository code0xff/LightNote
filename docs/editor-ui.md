...
Cards are **reordered by dragging**, on pointer events rather than HTML5 drag events (`editor/dragging.ts`): the HTML5 ones fire for mouse only, so touch had no way to reorder anything. The list rearranges live under the pointer rather than showing an insertion marker that predicts where the card will land, and nothing is written until the pointer is released. `documentsBeforeDrag` holds the order to restore, because a cancelled drag has to put it back — a release clears it first, and that is what tells the two apart.

The details that make it work, each of which was a bug first:

- **Move and release are tracked on `window`, with no `setPointerCapture`.** Reordering moves the dragged card in the DOM, which loses a capture held by that card: the release was never seen, so the new order sat on screen unsaved. Window listeners see the whole gesture no matter what the DOM does.
- **Touch drags start on the grip and nowhere else.** A finger dragging a card is the same gesture as scrolling the list, and the list has to keep scrolling; `touch-none` on the grip is what lets the drag win once it does start. A mouse has no such conflict, so it can take hold of the whole card.
- **A press only becomes a drag after `DRAG_ACTIVATION_DISTANCE`**, or every click on a card would arm a reorder.
- **Every button on a card asks `consumeDragClick` first.** A drag ends in a click on whatever the pointer was over, which would otherwise open the document it was dropped on — or press the delete button it was dropped on.
- **The list scrolls itself near its edges** (`edgeScrollStep`), on a frame loop rather than on pointer moves, because a finger held still at the edge sends no events. Without it a card can only be moved as far as the list happens to be showing.
- `draggable` is gone, and with it the Firefox quirk that needed `dataTransfer.setData` before a drag would start at all.

Arrow keys on the grip do the same move, since the drag itself is unreachable from the keyboard. The grip is a reorder control and nothing else, which is why it takes plain arrows rather than a modifier.
